...
interface SlideData {
  title: string
  button: string
  buttonUrl: string // URL para o redirecionamento
  src: string
}
...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  const { src, button, buttonUrl, title } = slide
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        <div
          className="absolute left-0 top-0 h-full w-full overflow-hidden rounded-[1%] bg-transparent transition-all duration-150 ease-out"
          style={{
            transform:
              current === index
        ...
        >
          <img
            className="duration-600 absolute inset-0 h-[100%] w-[100%] rounded-2xl object-contain opacity-100 transition-opacity ease-in-out"
            style={{
        ...
        >
          ...
          <div className="flex justify-center">
            <a
              href={buttonUrl}
              target="_blank" // Para abrir a URL em uma nova aba
...
export function Carousel({ slides }: CarouselProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[calc(80%+1rem)] flex w-full justify-center">
        <CarouselControl
          type="previous"
